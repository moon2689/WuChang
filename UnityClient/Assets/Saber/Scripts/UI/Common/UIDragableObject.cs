...
namespace Saber.UI
{
    [AddComponentMenu("UI/Dragable Object", 82)]
    public class UIDragableObject : UIBehaviour, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
    {
        ...
        public void OnBeginDrag(PointerEventData data)
        {
            ...
            this.m_TargetStartPosition = this.m_Target.position;
            this.m_Velocity = Vector2.zero;
        ...
        public void OnDrag(PointerEventData data)
        {
            ...
            {
                newPosition.x = this.m_Target.position.x;
            }
            if (!this.m_Vertical)
            {
                newPosition.y = this.m_Target.position.y;
            }

            // Apply the position change
            this.m_Target.position = newPosition;
        ...
        protected virtual void LateUpdate()
        {
            ...
            {
                Vector3 to = (this.m_Target.position - this.m_LastPosition) / Time.unscaledDeltaTime;
            ...
            if (!this.m_Dragging && this.m_Velocity != Vector2.zero)
            {
                ...
                for (int i = 0; i < 2; i++)
                {
                    ...
                    if (this.m_Inertia)
                    {
                        position[i] += this.m_Velocity[i] * Time.unscaledDeltaTime;
                    }
                    else
                    {
                ...
                if (this.m_Velocity != Vector2.zero)
                {
                    ...
                    if (!this.m_Horizontal)
                    {
                        position.x = this.m_Target.position.x;
                    }
                    if (!this.m_Vertical)
                    {
                    ...
                    // Apply the inertia
                    if (position != this.m_Target.position)
                    {
                        switch (this.m_InertiaRounding)
                        {
                            case Rounding.Hard:
                                this.m_Target.position = new Vector2(Mathf.Round(position.x / 2f) * 2f, Mathf.Round(position.y / 2f) * 2f);
                                break;
                            case Rounding.Soft:
                            default:
                                this.m_Target.position = new Vector2(Mathf.Round(position.x), Mathf.Round(position.y));
                                break;
                        }
                    }
                }
