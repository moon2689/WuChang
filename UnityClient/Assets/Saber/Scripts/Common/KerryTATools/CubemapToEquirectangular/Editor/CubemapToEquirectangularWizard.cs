...
class CubemapToEquirectangularWizard : ScriptableWizard
{
    ...
    void OnWizardCreate()
    {
        ...
        {
            UnityEngine.Debug.LogWarning("Unable to find shader");
            valid = false;
        }
        else
        {
        ...
        if (cubemap == null)
        {
            UnityEngine.Debug.LogWarning("You must specify a cubemap");
            valid = false;
        }
        else if (outputWidth < 1)
        {
            UnityEngine.Debug.LogWarning("Width must be greater than 0");
            valid = false;
        }
        else if (outputHeight < 1)
        {
            UnityEngine.Debug.LogWarning("Height must be greater than 0");
            valid = false;
        }
        ...
        if (valid)
        {
            ...
            UnityEngine.Debug.Log("Equirectangular asset successfully saved to " + textureAsset);
