...
namespace vietlabs.fr2
{
 ...
 typeof(Selection).GetProperty("assetGUIDs", BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static);
		...
		internal static void ExportSelection()
		{
			...
			if (packageExportT == null)
			{
				UnityEngine.Debug.LogWarning("Export Package Error : UnityEditor.PackageExport not found !");
				return;
			}
			...
			if (fieldInfo == null)
			{
				UnityEngine.Debug.LogWarning("Export Package error : " + prop + " not found !");
				return;
			}
			...
			if (methodInfo == null)
			{
				UnityEngine.Debug.LogWarning("Export Package error : BuildAssetList method not found !");
				return;
			}
