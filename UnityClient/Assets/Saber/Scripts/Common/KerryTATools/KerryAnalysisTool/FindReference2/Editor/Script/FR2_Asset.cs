...
namespace vietlabs.fr2
{
	...
	public class FR2_Asset
	{
		...
		internal void LoadFileInfo()
		{
			...
			if (IsMissing)
			{
				UnityEngine.Debug.LogWarning("Should never be here! - missing files can not trigger LoadFileInfo()");
				return;
			}
		...
		internal static List<string> FindUsageGUIDs(FR2_Asset asset, bool includeScriptSymbols)
		{
			...
			if (asset == null)
			{
				UnityEngine.Debug.LogWarning("Asset invalid : " + asset.m_assetName);
		...
		internal bool ReplaceReference(string fromGUID, string toGUID, TerrainData terrain = null)
		{
			...
			if (IsReferencable)
			{
				...
				catch (Exception e)
				{
					state = FR2_AssetState.MISSING;
//#if FR2_DEBUG
					UnityEngine.Debug.LogWarning("Replace Reference error :: " + e + "\n" + m_assetPath);
//#endif
				}
			...
			UnityEngine.Debug.LogWarning("Something wrong, should never be here - Ignored <" + m_assetPath +
        ...
        {
            UnityEngine.Debug.Log("ReplaceReference: from " + fromGUID + "  to: " + toGUID + "  toFileId: " + toFileId);
            if (IsMissing)
            {
                        ...
                        if(line.IndexOf(fromGUID, StringComparison.Ordinal) >= 0)
                        {
							...
							if(toFileId > 0)
							{
	                            ...
	                            if(index >= 0)
	                            {
									...
									{
                                        UnityEngine.Debug.LogWarning("cannot parse file");
                                        return false;
									}
                                    if (fileType.ToString().Substring(0,3) != toFileId.ToString().Substring(0,3))
                                    {
                                        //difference file type
                                        UnityEngine.Debug.LogWarning("Difference file type");
                                        return false;
                                    }

                                    UnityEngine.Debug.Log("ReplaceReference: fromFileId " + fromFileId + "  to File Id " + toFileId);
