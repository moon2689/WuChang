...
public class SceneErrorChecker : EditorWindow {
	...
	void CheckAllGameObjects() {
		...
		if (sceneRoot == null)
		{
			UnityEngine.Debug.LogError("场景中必须存在map根节点");
			return;
		}
		else
		{ 
	...
	{ 
		...
		{ 
			...
			if(mat == null)
				UnityEngine.Debug.LogError(renderer.gameObject.name + "丢失材质球", renderer.gameObject);
			else
	...
	{ 
		if(mat.shader != null)
		{
			if (!mat.shader.name.Contains("Qtz"))
				UnityEngine.Debug.LogError(mat.name + "材质球必须要使用Qtz组下的Shader", mat);
		}
		else
		{
			UnityEngine.Debug.LogError(mat.name + "材质球的Shader丢失", mat);
		}
	}
	...
	private void CheckGameObjectFlag(GameObject go)
	{
		if(GameObjectUtility.AreStaticEditorFlagsSet(go,StaticEditorFlags.ContributeGI) && !GameObjectUtility.AreStaticEditorFlagsSet(go,StaticEditorFlags.BatchingStatic))
			UnityEngine.Debug.LogError(go.name + "勾选了lightmapStatic却没有勾选BatchingStatic！！！", go);
	}
	//检查场景物件分组情况
	private void CheckGroupManage()
	{
		...
		if (staticRoot == null)
		{
			UnityEngine.Debug.LogWarning("场景中没有找到staticObjs，建议按照map/staticObjs对静态物体进行分类");
		}

		GameObject dynamicRoot = GameObject.Find("dynObjs");
		if (dynamicRoot == null)
		{
			UnityEngine.Debug.LogWarning("场景中没有找到dynObjs，建议按照map/dynObjs对非静态物体进行分类");
		}
	}
	...
	private void CheckHelpRootSetting()
	{
		...
		else
		{
			...
			{
				if (renderer.enabled)
					UnityEngine.Debug.LogError(string.Format("请禁用HelpRoot下{0}物体的MeshRenderer组件", renderer.gameObject.name), renderer.gameObject);
			}
		}
		...
		foreach (Light light in Object.FindObjectsOfType<Light>()) {
			if (light.enabled && light.cullingMask == -1) {
				if(light.lightmapBakeType != LightmapBakeType.Realtime && !light.bakingOutput.isBaked)
					UnityEngine.Debug.LogError(light.name + ": 该灯光还没有参与烘焙，请删除或重新烘焙", light);
				else if (light.lightmapBakeType == LightmapBakeType.Realtime)
					UnityEngine.Debug.LogError(light.name + ": 该灯光为实时光", light);
			}
		}
	...
	void CheckBakeSettings() {
		...
		if (Lightmapping.realtimeGI)
			UnityEngine.Debug.LogError("[Lighting面板] 必须关闭Realtime Global Illumination模式！");

		if (!Lightmapping.bakedGI)
			UnityEngine.Debug.LogError("[Lighting面板] 必须勾选Baked GI模式！");

		if (Lightmapping.giWorkflowMode != Lightmapping.GIWorkflowMode.OnDemand)
			UnityEngine.Debug.LogError("[Lighting面板] 请关闭Auto模式！");

		if (LightmapSettings.lightmaps.Length > 2)
		{
			UnityEngine.Debug.LogError("[Lighting面板] lightmap贴图最多为两张");
		}
        //if (LightmapEditorSettings.maxAtlasSize != 2048)
        if (Lightmapping.lightingSettings.lightmapMaxSize != 2048)
			UnityEngine.Debug.LogError("[Lighting面板] 建议将Lightmapping Settings -> Lightmap Size设置为2048");

		if (LightmapSettings.lightmapsMode != LightmapsMode.NonDirectional)
			UnityEngine.Debug.LogError("[Lighting面板] 必须将Directional Mode设置为Non-Directional");

		if (RenderSettings.fog) {
			if (RenderSettings.fogMode != FogMode.Linear) {
				UnityEngine.Debug.LogError("[Lighting面板] Fog->Fog Mode使用线性雾！");
			}
		}
	}
			...
			if (!EditorUtility.DisplayCancelableProgressBar("Checking UV 2", mesh.name, index++ / (float)sharedMeshes.Count)) {
				for (int i = 0; i < mesh.uv.Length; i++) {
					...
					if (lightmapUV.x < 0 || lightmapUV.x < 0 || lightmapUV.y > 1 || lightmapUV.y > 1) {
						UnityEngine.Debug.LogError(mesh.name + ": uv2不在[0, 1]范围内！", mesh);
						break;
					}
				}
			} else {
