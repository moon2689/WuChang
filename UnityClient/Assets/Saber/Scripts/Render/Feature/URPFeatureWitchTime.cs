...
namespace Saber
{
    public class URPFeatureWitchTime : ScriptableRendererFeature
    {
        class URPPassWitchTime : ScriptableRenderPass
        {
            Settings m_Settings;
            RenderTargetIdentifier m_source;
            RenderTargetIdentifier m_dest;
            int m_TempRTName = Shader.PropertyToID($"{nameof(URPFeatureWitchTime)}_TempRT");

            public URPPassWitchTime(Settings settings)
            {
                m_Settings = settings;
            }
            ...
            public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
            {
                ...
                CommandBuffer cmd = CommandBufferPool.Get($"{nameof(URPFeatureWitchTime)}_cmd");
        ...
        URPPassWitchTime m_Pass;


        public override void Create()
        {
            m_Pass = new URPPassWitchTime(m_Settings);
        ...
        {
            //s_IsActive = true;
            if (s_IsActive && m_Settings.m_Mat)
            {
