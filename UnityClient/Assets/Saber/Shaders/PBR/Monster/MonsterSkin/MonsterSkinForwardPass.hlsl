...
Varyings LitPassVertex(Attributes input)
{
    ...
    output.color = 1;
    #if _ONHURT_ON
    float3 dir = positionWS - _HurtPos.xyz;
    float lengthDir = length(dir); 
    if (lengthDir < _HurtRadius)
    {
        positionWS += (_HurtRadius - lengthDir) * sin(_Time.w * 20) * 0.3;
...
half4 LitPassFragment(Varyings input) : SV_Target0
{
    ...
    SurfaceData surfaceData = InitializeStandardLitSurfaceData(input.uv, input.color);
