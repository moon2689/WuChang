...
half4 LitPassFragment(Varyings input)
{
    ...
    SurfaceData surfaceData = InitializeStandardLitSurfaceData(input.uv, input.color.b, maskMap);
    InputData inputData = InitializeInputData(input, surfaceData.normalTS);

    float sgn = input.tangentWS.w;      // should be either +1 or -1
    float3 bitangent = sgn * cross(input.normalWS.xyz, input.tangentWS.xyz);
    HairSurfaceData hairSurfaceData = BuildHairSurfaceData(inputData, maskMap, bitangent, input.color, surfaceData.smoothness);
    
    half4 color = UniversalFragmentPBR(inputData, surfaceData, hairSurfaceData);
...
half4 LitPassFragmentClip(Varyings input) : SV_Target0
{
    UNITY_SETUP_INSTANCE_ID(input);
    UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(input);
