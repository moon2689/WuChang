...
CottonSurfaceData InitializeCottonSurface(float2 uv, half4 maskMap, half occlusion)
{
    float2 uvFuzzMap = uv * _FuzzMap_ST.xy + _FuzzMap_ST.zw;
    half4 fuzzMap = SAMPLE_TEXTURE2D(_FuzzMap, sampler_FuzzMap, uvFuzzMap);
    CottonSurfaceData surfaceData = (CottonSurfaceData)0;
    surfaceData.sheenColor = fuzzMap.rgb * _SheenColor.rgb;
    surfaceData.sheenDFG = maskMap.r;
...
half3 LightingPhysicallyBased(BRDFData brdfData, Light light, half3 normalWS, half3 viewDirectionWS, CottonSurfaceData cottonSurfaceData)
{
    ...
    //half3 brdf = (brdfData.diffuse + brdfSpec) * (1 - lossEnergy) + brdfSheenSpec;
