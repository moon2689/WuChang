...
public static class TerrainToMesh
{
    ...
    static void ConvertTerrainToMesh()
    {
        ...
        int vertexCountScale = 8; //将顶点数稀释 vertexCountScale*vertexCountScale 倍
        ...
        Vector2 uvScale = new Vector2(1f / (w - 1f), 1f / (h - 1f)) * vertexCountScale *
                          (size.x / terrainData.terrainLayers[0].tileSize.x); //此处有问题，若每个图片大小不一，则出问题。日后改善

        w = (w - 1) / vertexCountScale + 1;
        h = (h - 1) / vertexCountScale + 1;
        Vector3[] vertices = new Vector3[w * h];
        Vector2[] uvs = new Vector2[w * h];
        Vector4[] alphasWeight = new Vector4[w * h]; // 只支持4张图片
        ...
        string sceneFolder = $"Assets/Saber/Scenes/{sceneName}";
        string meshPath = $"{sceneFolder}/{sceneName}.asset";
        AssetDatabase.CreateAsset(mesh, meshPath);
        AssetDatabase.SaveAssets();
        mesh = AssetDatabase.LoadAssetAtPath<Mesh>(meshPath);

        string transName = "TerrainMesh";
        GameObject goMesh = GameObject.Find(transName);
        if (goMesh == null)
        {
            goMesh = new GameObject(transName, typeof(MeshFilter), typeof(MeshRenderer), typeof(MeshCollider));
        }
        ...
        if (!mat)
        {
            string matPath = $"{sceneFolder}/TerrainMesh{sceneName}.mat";
        ...
        for (int i = 0; i < terrainData.terrainLayers.Length; i++)
        {
            ...
            mat.SetTexture("_Texture" + i, terrainLayer.diffuseTexture);
        }

        Transform t = goMesh.transform;
